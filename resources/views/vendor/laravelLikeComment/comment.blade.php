...
?>
<div class="laravelComment" id="laravelComment-{{ $comment_item_id }}" style="clear: both;padding-top: 20px;">
    ...
    <div class="ui threaded comments" id="{{ $comment_item_id }}-comment-0">
        ...
        <form style="clear: both;padding-top: 10px;" class="ui laravelComment-form form" id="{{ $comment_item_id }}-comment-form" data-parent="0" data-item="{{ $comment_item_id }}" style="display: none;">
            <div class="field">
                <textarea id="0-textarea" rows="2" {{ $GLOBALS['commentDisabled'] }}></textarea>
                @if(!Auth::check())
                    <small style="clear: both;padding-top: 10px;">Please Log in to comment</small>
                @endif
            </div>
